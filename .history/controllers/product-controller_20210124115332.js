...
const GetProductDetailsByName = (request, response) => {
    try {
        ...
        const books = res.body;
        var Matched_books = []
        var re = new RegExp(request.query.title, 'i');
        books.forEach(function(item, ix) {
            Object.keys(item).forEach(function(key) {
              if (typeof item[key] !== 'string') return;
              if (item[key].match(re)) {
                if (found.indexOf(ix) === -1) { Matched_books.push(ix); }
              }
            });
        if(Matched_books)
            return response.json({Products:Matched_books,ProductCount:Matched_books.length});
        else    
            return json({err:"Not Found"})
